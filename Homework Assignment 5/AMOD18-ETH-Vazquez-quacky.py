...
#        ret,label,center=cv2.kmeans(Z,K,None,criteria,10,cv2.KMEANS_RANDOM_CENTERS)
        center, label = self.kmeans_custom(Z, K)
    ...
    def kmeans_custom(self, Z, K=2, num_iter=10):
        num_pix, num_rgb_chan = np.shape(Z);
        # Generate random center points of shape [K, 3]
        centers = np.random.randint(0,255, size=(K, num_rgb_chan))
        assigned_clusters = np.zeros((num_pix, 1))
        for i in range(num_iter):
            for j in range(num_pix):
                distances = self.distance_calc(Z[j,:], centers)    # distance to each centr [no_cluster, ]
                assigned_cluster = np.argmin(distances)
                assigned_clusters[j] = assigned_cluster
            for j in range(K):
                points = [Z[k,:] for k in range(num_pix) if assigned_clusters[k] == j]
                centers[j] = np.mean(points, axis=0)
                
        return centers.astype(int), assigned_clusters.astype(int)

    def distance_calc(self, a, b, ax=1):
